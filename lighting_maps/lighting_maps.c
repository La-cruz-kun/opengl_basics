...
// Interleaved data: [POS.X, POS.Y, POS.Z, NORM.X, NORM.Y, NORM.Z, TEX.U, TEX.V]
    ...
    Shader shader = {"lighting_maps/vertex.glsl", "lighting_maps/fragment.glsl", 0};
    Shader lightShader = {"lighting_maps/light_vertex.glsl", "lighting_maps/light_fragment.glsl", 0};
    ShaderInit(&shader);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glm_translate(modelCube, cubePosition[0]);
        ...
        ShaderUse(shader);
        glm_mat4_identity(modelCube);
        glm_translate(modelCube, (vec3) {0, -1, 0});
