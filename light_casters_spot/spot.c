...
// Interleaved data: [POS.X, POS.Y, POS.Z, NORM.X, NORM.Y, NORM.Z, TEX.U, TEX.V]
  ...
  vec3 cubePositions[] = {
    ...
    Shader shader = {"light_casters_spot/vertex.glsl", "light_casters_spot/fragment.glsl", 0};
    Shader lightShader = {"light_casters_spot/light_vertex.glsl", "light_casters_spot/light_fragment.glsl", 0};
    ShaderInit(&shader);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUniform3f(glGetUniformLocation(shader.ID, "light.position"), camera.Position[0], camera.Position[1], camera.Position[2]);
        glUniform3f(glGetUniformLocation(shader.ID, "light.direction"), camera.Front[0], camera.Front[1], camera.Front[2]);
        glUniform1f(glGetUniformLocation(shader.ID, "light.cutoff"), cos(glm_rad(12.5)));
        glUniform1f(glGetUniformLocation(shader.ID, "light.outerCutoff"), cos(glm_rad(20)));
        glUniform1f(glGetUniformLocation(shader.ID, "light.constant"), 1);
        glUniform1f(glGetUniformLocation(shader.ID, "light.linear"), 0.07);
        glUniform1f(glGetUniformLocation(shader.ID, "light.quadratic"), 0.017);

        glUniform3f(glGetUniformLocation(shader.ID, "light.ambient"), 0.05, 0.05, 0.05);
