...
// Interleaved data: [POS.X, POS.Y, POS.Z, NORM.X, NORM.Y, NORM.Z, TEX.U, TEX.V]
  ...
  vec3 cubePositions[] = {
    ...
    Shader shader = {"light_casters_directional/vertex.glsl", "light_casters_directional/fragment.glsl", 0};
    Shader lightShader = {"light_casters_directional/light_vertex.glsl", "light_casters_directional/light_fragment.glsl", 0};
    ShaderInit(&shader);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUniform3f(glGetUniformLocation(shader.ID, "light.direction"), -0.2, -1, -0.3);
        ...
        glm_scale(modelCube, (vec3) {5, 1, 5});
        glUniformMatrix4fv(glGetUniformLocation(shader.ID, "model"), 1, GL_FALSE,
                 &modelCube[0][0]);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, diffuse_data);
